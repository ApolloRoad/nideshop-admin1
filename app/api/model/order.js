...
module.exports = class extends think.Model {
  ...
  getOrderHandleOption(orderId) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const orderInfo = yield _this.where({ id: orderId }).find();
  ...
  getOrderStatusText(orderId) {
    ...
    return _asyncToGenerator(function* () {
      const orderInfo = yield _this2.where({ id: orderId }).find();
      let statusText = '未付款';
  ...
  updatePayStatus(orderId, payStatus = 0) {
    ...
    return _asyncToGenerator(function* () {
      return _this3.where({ id: orderId }).limit(1).update({ pay_status: parseInt(payStatus) });
    })();
  }
  ...
  getOrderByOrderSn(orderSn) {
    ...
    return _asyncToGenerator(function* () {
      ...
      return _this4.where({ order_sn: orderSn }).find();
    })();
  }
};
